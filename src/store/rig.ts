...
export const useRigStore = defineStore('rig', {
  ...
  actions: {
    ...
    async connect(host: string = 'localhost', port: number = 4532, model?: number, device?: string) {
      this.isLoading = true;
      this.error = null;
    ...
    async handleReconnect() {
      ...
      await this.connect(this.connection.host, this.connection.port, this.connection.model, this.connection.device);
    },
    ...
    parseCapabilities(capData: string[]): RigCapabilities {
      ...
      for (const line of capData) {
        if (line.startsWith('Model name:')) {
          ...
          caps.modes = line.split('\t')[1].split(' ').filter(m => m.length > 0);
        } else if (line.startsWith('VFO list:')) {
          caps.vfos = line.split('\t')[1].split(' ').filter(v => v.length > 0);
        } else if (line.startsWith('Get functions:')) {
          caps.functions = line.split('\t')[1].split(' ').filter(f => f.length > 0);
        } else if (line.startsWith('Get level:')) {
          caps.levels = line.split('\t')[1].split(' ').filter(l => l.length > 0);
        }
      }
